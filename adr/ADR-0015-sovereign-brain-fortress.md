...
4. **EXECUTION_PROVIDER_SANDBOX** — `immune_execution_broker` possui subprocesso; `immune_provider_proxy` possui transporte externo, DLP, quarentena de prompt injection e credencial do provider.
...
6. **ADAPTERS** — allowlist de ações e risco pertence ao adapter homologado; adapter não recebe soberania.
...
- Provider live passa exclusivamente pelo Proxy em runner isolado;
