...
write(
    "tests/phase1/validate_phase1.py",
    dedent(
        """
        #!/usr/bin/env python3
        from __future__ import annotations
        ...
        expected_contracts = {
            "system.schema.json", "mission.schema.json", "incident.schema.json", "task.schema.json",
            "worker.schema.json", "evidence.schema.json", "checkpoint.schema.json",
            "policy-decision.schema.json", "donor-component.schema.json", "human-exception.schema.json",
        }
        ...
        for path in sorted(contract_dir.glob("*.schema.json")):
            try:
                ...
                ok(f"strict_object:{path.name}", data.get("type") == "object" and data.get("additionalProperties") is False)
        ...
        policy_decision = json.loads((contract_dir / "policy-decision.schema.json").read_text(encoding="utf-8"))
        decisions = set(policy_decision["properties"]["decision"]["enum"])
        ok("policy_decisions_exact", decisions == {"PERMITIR", "PERMITIR_COM_RESTRIÇÕES", "EXIGIR_CHECKPOINT", "EXIGIR_APROVAÇÃO_HUMANA", "BLOQUEAR", "CONTER_E_ESCALAR"})

        # State machines: exact normative states and key transition guards must be materialized.
        ...
        for state in ["CREATED", "AUTHORIZED", "DISCOVERING", "RUNNING", "DEGRADED", "BLOCKED", "WAITING_HUMAN", "CONTAINED", "VALIDATING", "COMPLETED", "FAILED_SAFE", "CANCELLED"]:
            ok(f"mission_state:{state}", state in mission)
        for state in ["DETECTED", "CONFIRMED", "TRIAGED", "INVESTIGATING", "CAUSE_IDENTIFIED", "FIX_PLANNED", "FIX_TESTING", "FIX_APPROVED", "APPLYING", "VALIDATING", "MONITORING_RECOVERY", "RESOLVED", "ROLLED_BACK", "CONTAINED", "ESCALATED", "CLOSED_WITH_RISK"]:
        ...
        ok("attempt_no_blind_retry", "equivalent_retry_requires_new_evidence" in attempt and "each_retry_requires_technical_delta" in attempt)

        # Constitution and policy fail-closed invariants.
        dna = (ROOT / "constitution/IMUNE-DNA-001.md").read_text(encoding="utf-8")
        for marker in ["Fail-closed", "Open Source Only", "Separação de funções", "Checkpoint", "MISSION_PROVEN", "Loop Engineering"]:
        ...
        ok("authority_fail_closed", 'default decision := "BLOQUEAR"' in (ROOT / "policies/authority.rego").read_text(encoding="utf-8"))
        ok("donor_fail_closed", 'default decision := "BLOQUEAR"' in (ROOT / "policies/donor-oss.rego").read_text(encoding="utf-8"))
        ok("mission_proven_default_false", "default mission_proven := false" in (ROOT / "policies/mission-proven.rego").read_text(encoding="utf-8"))

        # Contract-level acceptance scenarios. Runtime PolicyGuard implementation is Phase 2.
        def authority(i):
            return "PERMITIR" if all(i.get(k) is True for k in ("mission_authorized", "system_authorized", "requester_authorized", "scope_ok")) else "BLOQUEAR"
        def financial(i):
            return "EXIGIR_APROVAÇÃO_HUMANA" if any(i.get(k) is True for k in ("new_cost", "purchase", "subscription", "trial_with_billing_risk", "commercial_license")) else "PERMITIR"
        def material(i):
            ...
            return "PERMITIR"
        def donor(i):
            gates = ("open_source", "license_verified", "origin_pinned", "artifact_hash_verified", "security_scanned", "laboratory_approved")
            return "PERMITIR_COM_RESTRIÇÕES" if all(i.get(k) is True for k in gates) and i.get("authority") == "adapter-only" else "BLOQUEAR"
        def mission_proven(i):
            gates = ("scope_explicit", "observable_result_achieved", "relevant_tests_passed", "regression_validated", "recovery_validated", "security_validated", "evidence_preserved", "no_critical_blocker", "independent_audit_passed")
            return all(i.get(k) is True for k in gates)

        scenarios = {
            "invalid_mission_blocked": authority({}) == "BLOQUEAR",
            "worker_scope_expansion_blocked": authority({"mission_authorized": True, "system_authorized": True, "requester_authorized": True, "scope_ok": False}) == "BLOQUEAR",
            "paid_action_requires_human": financial({"purchase": True}) == "EXIGIR_APROVAÇÃO_HUMANA",
            "material_change_without_checkpoint_blocked": material({"material_change": True}) == "EXIGIR_CHECKPOINT",
            "irreversible_without_recovery_requires_human": material({"irreversible": True}) == "EXIGIR_APROVAÇÃO_HUMANA",
            "disable_security_control_blocked": material({"disables_security_control": True}) == "BLOQUEAR",
            "non_oss_donor_blocked": donor({"open_source": False}) == "BLOQUEAR",
            "oss_donor_without_lab_blocked": donor({"open_source": True, "license_verified": True, "origin_pinned": True, "artifact_hash_verified": True, "security_scanned": True, "laboratory_approved": False, "authority": "adapter-only"}) == "BLOQUEAR",
            "mission_proven_missing_gate_false": mission_proven({"scope_explicit": True}) is False,
            "mission_proven_all_gates_true": mission_proven({k: True for k in ("scope_explicit", "observable_result_achieved", "relevant_tests_passed", "regression_validated", "recovery_validated", "security_validated", "evidence_preserved", "no_critical_blocker", "independent_audit_passed")}) is True,
        }
        for name, passed in scenarios.items():
            ok(f"scenario:{name}", passed)

        # Donor inventory already collected must remain OSS-only and authority-less by default.
        ...
        # Hash all Phase 1 normative inputs for reproducible evidence.
        controlled_roots = ["specification", "constitution", "contracts", "state-machines", "policies", "acceptance", "adr"]
        hashes = {}
        ...
        evidence = {
            ...
            "source_sha256": json.loads((ROOT / "specification/SOURCE_MANIFEST.json").read_text(encoding="utf-8"))["sha256"],
            "checks": checks,
            "summary": {"total": len(checks), "passed": sum(1 for c in checks if c["passed"]), "failed": len(failures)},
            "controlled_file_sha256": hashes,
            "result": "PHASE1_PROVEN" if not failures else "PHASE1_FAILED",
            "scope_note": "PHASE1_PROVEN proves specification/contracts consistency only; it is not MISSION_PROVEN for the complete product.",
        }
        ...
        status.write_text(
            ...
            + ("**Estado: PHASE1_PROVEN**\n" if not failures else "**Estado: PHASE1_FAILED**\n")
