...
from immune_core.discovery import DiscoveryEngine,TCPHealthSensor
...
required=["immune_core/observability.py","immune_core/discovery.py","immune_gateway/adapters.py","config/gateway-runtime.json","tests/phase5/test_discovery.py","scripts/validate_phase5.py","adr/ADR-0005-discovery-observability.md",".github/workflows/phase5-observability.yml"]
for rel in required: ok(f"artifact:{rel}",(ROOT/rel).is_file())
for rel in ("immune_core/observability.py","immune_core/discovery.py","immune_gateway/adapters.py"):
    try: ast.parse((ROOT/rel).read_text(encoding="utf-8")); ok(f"ast:{rel}",True)
    except SyntaxError as exc: ok(f"ast:{rel}",False,exc)
obs_text=(ROOT/"immune_core/observability.py").read_text(encoding="utf-8"); disc_text=(ROOT/"immune_core/discovery.py").read_text(encoding="utf-8"); gateway_text=(ROOT/"immune_gateway/adapters.py").read_text(encoding="utf-8"); gateway_cfg=json.loads((ROOT/"config/gateway-runtime.json").read_text(encoding="utf-8"))
ok("vendor_neutral_models","ObservabilityStore" in obs_text and "SignalProcessor" in obs_text); ok("signal_deduplication","recent_fingerprint_count" in obs_text); ok("signal_correlation","recent_correlation_count" in obs_text); ok("robust_baseline_anomaly","statistics.median" in obs_text and "mad" in obs_text.lower()); ok("evidence_sha256","payload_sha256" in obs_text and "verify_evidence" in obs_text); ok("structured_metrics","obs_metrics" in obs_text); ok("structured_logs","obs_logs" in obs_text); ok("structured_traces","obs_traces" in obs_text); ok("sensor_health_isolated","consecutive_failures" in obs_text and "continue" in disc_text); ok("dependency_graph","DependencyGraph" in obs_text);
try:
    TCPHealthSensor("direct-network-forbidden","127.0.0.1",9,timeout_seconds=0.01)
    direct_tcp_blocked=False
except RuntimeError:
    direct_tcp_blocked=True
ok("direct_tcp_health_fail_closed",direct_tcp_blocked)
ok("protected_tcp_health_gateway_owned","class TCPHealthGatewayAdapter" in gateway_text and "observation-only" in gateway_text)
ok("default_gateway_has_no_real_targets",gateway_cfg.get("systems")==[])
ok("donor_adapter_gate",'decision_value != "approved"' in disc_text and "adapter-only" in disc_text and "executable" in disc_text)
